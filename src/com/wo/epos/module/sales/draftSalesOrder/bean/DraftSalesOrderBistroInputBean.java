...
@ManagedBean(name = "draftSalesOrderBistroInputBean")
@ViewScoped
public class DraftSalesOrderBistroInputBean extends CommonBean implements Serializable {

	private static final long serialVersionUID = 7727870677960860773L;
	static Logger logger = Logger.getLogger(DraftSalesOrderBistroInputBean.class);
	...
	public void saveTemporary(){
		try{  
			 if(MODE_TYPE.equals(CommonConstants.MODE_TYPE_ADD))
			 {
				 ...
				 draftSalesOrderService.saveSoDistro(salesOrderVoHeader, salesOrderDtlVOList, userSession.getUserCode());	
			 ...
			 {
				 draftSalesOrderService.updateSoDistro(salesOrderVoHeader, salesOrderDtlVOList, userSession.getUserCode());
		...
		catch(Exception ex)
		{
			...
			facesUtils.addFacesMsg(
					...
					null);
			draftSalesOrderService.rollback();
		}
	}
			  ...
			  if(validatePayment()){
				  if(salesOrderVoHeader.getStatusCode() !=null && !salesOrderVoHeader.getStatusCode().equals(""))
				  {
					  if(!salesOrderVoHeader.getStatusCode().equals(CommonConstants.SO_BILL))
					  {
						  ...
						  draftSalesOrderService.savePayment(salesOrderVoHeader, salesOrderDtlVOList, dataPaymentTypeVoList, userSession.getUserCode());
						  draftSalesOrderService.flush();
						  buttonCancel();
					  }
					  else
					  {
						  draftSalesOrderService.saveSoDistroPaymentBill(salesOrderVoHeader, dataPaymentTypeVoList, userSession.getUserCode());					  
						  ...
						  if(invList.size() == 1){							  
							  ...
							  salesOrderVoHeader.setStatusCode(CommonConstants.SO_PAYMENT);
							  draftSalesOrderService.updateSoPayment(salesOrderVoHeader, userSession.getUserCode());
							  draftSalesOrderService.flush();
							  buttonCancel();
						  }
						  else
	...
	public void buttonSaveDialogTagihan(){
		try{  
			if(salesOrderDtlVOTagihanList.isEmpty())
			{
		         ...
		         salesOrderTagihan = draftSalesOrderService.saveSoDistroBill(salesOrderVoHeader, salesOrderDtlVOList, dataTableDialogTagihan, userSession.getUserCode());
		         draftSalesOrderService.flush();
	...
	public boolean modeEdit(EquipmentVO equipDialog){
		...
		if(equipDialog != null && equipDialog.getEquipmentId() !=null){
			DraftSalesOrderVO salesOrderVoEDit = draftSalesOrderService.searchDataSoByEquipment(equipDialog.getEquipmentId(), equipDialog.getEquipmentStatusCode());
			if(salesOrderVoEDit !=null){
				salesOrderVoHeader = salesOrderVoEDit;
				salesOrderVoHeader.setTitelDiscount("Diskon");
				
				if(salesOrderVoHeader.getStatusCode().equals(CommonConstants.SO_BILL)){
					salesOrderTagihan = new DraftSalesOrder();
	...
	public static void setLogger(Logger logger) {
		DraftSalesOrderBistroInputBean.logger = logger;
	}
