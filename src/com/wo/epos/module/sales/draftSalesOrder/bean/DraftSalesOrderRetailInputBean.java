...
import com.wo.epos.module.sales.draftSalesOrder.service.DraftSalesOrderRetailService;
...
@ManagedBean(name = "draftSalesOrderRetailInputBean")
@ViewScoped
public class DraftSalesOrderRetailInputBean extends CommonBean implements Serializable {

	private static final long serialVersionUID = 7843026683560754008L;

	static Logger logger = Logger.getLogger(DraftSalesOrderRetailInputBean.class);
	...
	@ManagedProperty(value = "#{draftSalesOrderRetailService}")
	private DraftSalesOrderRetailService draftSalesOrderRetailService;
		...
		try {
			if (validateRegister()) {
				if (validateSaveDirect()) {
					...
					draftSalesOrderRetailService.saveDirect(salesOrderVoHeader, salesOrderDtlVOList,
			...
			ex.printStackTrace();
			draftSalesOrderRetailService.rollback();
		}
	}
	...
	public void savePayment() {
		try {
			if (validatePayment()) {
				salesOrderVoHeader.setStatusCode(CommonConstants.SO_PAYMENT);
				salesOrderVoHeader.setSoTypeCode(CommonConstants.SO_PICKUP);
				draftSalesOrderRetailService.savePayment(salesOrderVoHeader, salesOrderDtlVOList, dataPaymentTypeVoList,
						userSession.getUserCode());
				buttonCancel();
	...
	public DraftSalesOrderRetailService getDraftSalesOrderRetailService() {
		return draftSalesOrderRetailService;
	}

	public void setDraftSalesOrderRetailService(DraftSalesOrderRetailService draftSalesOrderRetailService) {
		this.draftSalesOrderRetailService = draftSalesOrderRetailService;
	}
