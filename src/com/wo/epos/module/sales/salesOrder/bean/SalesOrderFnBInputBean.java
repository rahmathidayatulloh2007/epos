...
import com.wo.epos.module.sales.salesOrder.service.SalesOrderFnBService;
...
@ManagedBean(name = "salesOrderFnBInputBean")
@ViewScoped
public class SalesOrderFnBInputBean extends CommonBean implements Serializable {

	private static final long serialVersionUID = -4535698702549175765L;

	static Logger logger = Logger.getLogger(SalesOrderFnBInputBean.class);
	
	@ManagedProperty(value = "#{salesOrderFnBService}")
	private SalesOrderFnBService salesOrderFnBService;
	...
	public void saveTemporary(){
		try{  
			 if(MODE_TYPE.equals(CommonConstants.MODE_TYPE_ADD))
			 {
				 ...
				 salesOrderFnBService.saveSoDistro(salesOrderVoHeader, salesOrderDtlVOList, userSession.getUserCode());
			 ...
			 {
				 salesOrderFnBService.updateSoDistro(salesOrderVoHeader, salesOrderDtlVOList, userSession.getUserCode());
			...
			{  
				if(salesOrderVoHeader.getStatusCode() !=null && !salesOrderVoHeader.getStatusCode().equals("")){
					  if(!salesOrderVoHeader.getStatusCode().equals(CommonConstants.SO_BILL))
					  {
						  ...
						  salesOrderFnBService.savePayment(salesOrderVoHeader, salesOrderDtlVOList, dataPaymentTypeVoList, userSession.getUserCode());
						  buttonCancel();	
						  salesOrderFnBService.flush();
					  }
					  else
					  {
						  salesOrderFnBService.saveSoDistroPaymentBill(salesOrderVoHeader, dataPaymentTypeVoList, userSession.getUserCode());
						  salesOrderFnBService.flush();
	...
	public void buttonSaveDialogTagihan(){
		try{  
			if(salesOrderDtlVOTagihanList.isEmpty())
			{
		         ...
		         salesOrderTagihan = salesOrderFnBService.saveSoDistroBill(salesOrderVoHeader, salesOrderDtlVOList, dataTableDialogTagihan, userSession.getUserCode());
		         salesOrderFnBService.flush();
	...
	public boolean modeEdit(EquipmentVO equipDialog){
		...
		if(equipDialog.getEquipmentId() !=null){
		    SalesOrderVO salesOrderVoEDit = salesOrderFnBService.searchDataSoByEquipment(equipDialog.getEquipmentId(), equipDialog.getEquipmentStatusCode());
	...
	public static void setLogger(Logger logger) {
		SalesOrderFnBInputBean.logger = logger;
	}
	...
	public SalesOrderFnBService getSalesOrderFnBService() {
		return salesOrderFnBService;
	}

	public void setSalesOrderFnBService(SalesOrderFnBService salesOrderFnBService) {
		this.salesOrderFnBService = salesOrderFnBService;
	}
