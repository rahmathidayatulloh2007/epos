...
@ManagedBean(name="salesOrderDistributorService")
@ViewScoped
public class SalesOrderDistributorServiceImpl implements SalesOrderDistributorService, Serializable{
	...
	public void saveDirect(SalesOrderVO salesOrderVO,
	  ...
	  try{   
		    ...
		    updateDraftSo(salesOrderVO,salesOrderDtlVoList,user);
		    }
		    salesOrderDAO.flush();  
	...
	public List<DraftSalesOrderDtlVO> searchListSoDtlVO(Long soId) {
		...
		return draftSalesOrderDtlDAO.searchListSoDtlVO(soId);
	}

	@Override
	public List<DraftSalesOrderDtl> searchListSoDtl(Long soId) {
		// TODO Auto-generated method stub
		return draftSalesOrderDtlDAO.searchListSoDtl(soId);
	}

	@Override
	public void updateDraftSo(SalesOrderVO salesOrderVO,List<SalesOrderDtlVO> draftSalesOrderDtlVoList,
			String user) {
		DraftSalesOrder draftSalesOrder = draftSalesOrderDAO.findById(salesOrderVO.getSoId());
		System.out.println("size dtl=="+draftSalesOrder.getSalesOrderDtlList().size());
		//List<DraftSalesOrderDtl> listRemove = new ArrayList<>();
		for(int i=0;i<draftSalesOrder.getSalesOrderDtlList().size();i++){
			DraftSalesOrderDtl dtl = draftSalesOrder.getSalesOrderDtlList().get(i);
			Boolean flag = false;
			
			for(int x=0;x<draftSalesOrderDtlVoList.size();x++){
				SalesOrderDtlVO vo = draftSalesOrderDtlVoList.get(x);
				if(vo.getProductId().equals(dtl.getProductId())){
					flag = true;
				}
			}
			
			if(flag){
			draftSalesOrder.getSalesOrderDtlList().remove(i);
				//listRemove.add(dtl);
			}
		}
		
/*		for(int i=0;i<listRemove.size();i++){
			draftSalesOrder.getSalesOrderDtlList().remove(listRemove.get(i));
		}*/
		
		System.out.println("size dtl new=="+draftSalesOrder.getSalesOrderDtlList().size());
		
		if(draftSalesOrder.getSalesOrderDtlList().size()== 0 || draftSalesOrder.getSalesOrderDtlList().size() <=0){
			draftSalesOrderDAO.delete(draftSalesOrder);
			//draftSalesOrderDAO.flush();
		}else{
		draftSalesOrderDAO.update(draftSalesOrder);
		//draftSalesOrderDAO.flush();
		}
	}
	...
	public String runningNumberSo(String systemPropertyName, Long companyId) {
		// TODO Auto-generated method stub
		return salesOrderDAO.runningNumberSo(systemPropertyName, companyId);
	}
